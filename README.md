<h1 align="center">Hi there, I'm Ninad! 👋</h1>
<br>

## 🧩 Software Engineer | Full‑Stack & GenAI Developer  

I’m a Computer Engineering graduate who loves turning ideas into clean, functional software.  

**Core Focus:**  
Building scalable web applications through **Full‑Stack Development** — using **React**, **Next.js**, **TypeScript**, **Node.js**, **Express**, and **MongoDB** to craft reliable, maintainable systems.  

**Exploration:**  
Integrating **Generative AI** technologies like **LangChain**, **LangGraph**, **RAG**, and **Agentic AI workflows** to make web systems more intelligent and adaptive.  

---

### 💻 What I Do
- 🧩 Build full‑stack web applications using modern JavaScript frameworks.  
- ⚙️ Design APIs and backend logic with Node.js, Express, and FastAPI.  
- 🤖 Develop agentic AI workflows and RAG‑based systems for smarter automation.  
- 🧠 Work with databases (MongoDB, MySQL) and optimize data flow for reliability.  
- 🔧 Continuously refine code quality, scalability, and developer experience.

---

### 🧠 Projects
- [**Resumarq**](https://github.com/NINAD-17/Resumarq): GenAI resume–JD analysis tool performing ATS audits and impact scoring.  
- [**SSPM Community**](https://github.com/NINAD-17/SSPM-Community): A full‑stack platform connecting college communities.  
- [**Policy‑Guard**](https://github.com/NINAD-17/Policy-Guard): AI‑powered SOP compliance chatbot using vector search and LangChain.  

---

<table border="0">
  <tr>
    <td rowspan="2" width=45%>
      <img width=100% height=100% src="https://github.com/NINAD-17/NINAD-17/assets/94175390/36b73704-0188-4502-b228-122d68112b4b" />
    </td>
    <td>
      <img width=100% src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=NINAD-17&theme=github_dark" />
    </td>
  </tr>
  <tr>
    <td>
      <img width=100% src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=NINAD-17&theme=github_dark" />
    </td>
  </tr>
</table>

---

<h2 align="center">Tech Stack 👨🏻‍💻</h2>
<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,nodejs,express,mongodb,mysql,fastapi,docker,git,github,postman,c,cpp,python&perline=8" />
  </a>
</p>
<p align="center">
  <strong>AI & Agentic Tools:</strong> LangChain • LangGraph • Gemini API • RAG • Agents
</p>

---

<h2 align="left">
  Connect with me on <font color="blue">LinkedIn</font> 
  <a href="https://www.linkedin.com/in/ninad-dhulap/" target="blank">
    <img align="center" src="https://user-images.githubusercontent.com/88904952/234979284-68c11d7f-1acc-4f0c-ac78-044e1037d7b0.png" alt="linkedin" height="50" width="50" />
  </a>
  & 
  Twitter 
  <a href="https://x.com/Ninad__D" target="blank">
    <img align="center" src="https://user-images.githubusercontent.com/88904952/234980676-61bfb021-ecc8-48f7-88e6-34c1b06c4a58.png" alt="twitter" height="50" width="50" />
  </a>
</h2>

<h3 align="left">Thanks for stopping by! 😊</h3>
<br>
<p align="center">
  The character featured in this file is <strong>‘GNUSMΛS’</strong>, a creation of <strong>SΛMSUNG</strong>.  
  As a Samsung fan, I use it to represent curiosity and innovation — values that inspire my work.  
  If Samsung ever requests removal, I’ll respectfully comply.
</p>
